...
let results;
  ...
  const quiz = {
    ...
    try {
      const response = await fetch(`${GET_API}?tag=English`);
      if (!response.ok) {
    ...
    renderResults: function () {
      // Kiểm tra nếu `listResults` có giá trị hợp lệ
      if (listResults[currentIndex] !== undefined && quizAnswers[listResults[currentIndex]]) {
        quizAnswers.forEach((item) => {
          item.classList.remove("incorrect");
        });
        quizAnswers[listResults[currentIndex]].classList.add("active");
      } else {
        console.error("Error: Cannot access classList because the element is undefined.");
      }
    
      // Kiểm tra nếu đáp án chọn sai
      if (listSubmit[currentIndex] !== listResults[currentIndex] && quizAnswers[listSubmit[currentIndex]]) {
